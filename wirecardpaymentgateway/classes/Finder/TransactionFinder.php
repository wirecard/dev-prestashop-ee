...
class TransactionFinder extends DbFinder
{
    ...
    public function getCurrentTransactionByOrderId($orderId)
    {
        ...
        $query = $queryBuilder->from('wirecard_payment_gateway_tx')
            ->select('`' . _DB_PREFIX_ . 'wirecard_payment_gateway_tx`.`tx_id`')
            ->leftJoin('orders', 'o', '`' . _DB_PREFIX_ . 'wirecard_payment_gateway_tx`.`order_id` = o.`id_order`')
            ->leftJoin(
                'order_history',
                'order_history',
                '`' . _DB_PREFIX_ . 'wirecard_payment_gateway_tx`.`order_id` = order_history.`id_order`'
            )
            ->where(
